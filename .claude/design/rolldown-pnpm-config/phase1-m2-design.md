...
3. **Override box has no width guard.** The override box (`formatOverrideWarning`) was ported verbatim from Silk and, like Silk, pads with a bare `WARNING_BOX_WIDTH - len` (no `Math.max(0, …)`), so a `setting` path longer than ~71 chars throws at install time. The security box uses the guarded form. Non-blocking; carried to M3. See `package/src/runtime/warnings.ts`.
