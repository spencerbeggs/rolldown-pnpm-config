...
Builds **never write** — structurally, not by convention. A workspace-sourced catalog entry (`source: "workspace"`) is rewritten only by the `upgrade` CLI (`--yes` applies, `--check` gates); the build reads the config exactly as authored. An earlier iteration of this branch ran a workspace-source sync inside the memoized freeze path (`syncWorkspaceCatalogs`, with an `onCatalogUpdate` notification callback), but dogfood adoption by the first consumer showed it mutated the consumer's `savvy.build.ts` on every ordinary build once an entry declared `source: "workspace"`, ungated — the documented callback env-gate sat one layer too high (the sync wrote first; the callback only notified afterward). The fix was deleting the mutation path rather than gating it: with the CLI as the sole writer, the cross-repo spec's convergence requirement is still satisfied (its D7 was reversed upstream accordingly). The post-removal audit found the old shape also carried a second, latent **correctness** defect — the stronger argument for removal. The freeze path has exactly two consumers of `config.catalogs`: `normalizeCatalogs` (`package/src/plugin/freeze.ts:64`, feeding the emitted catalogs) and `resolvePeerDependencyRules` resolving the `allowedVersionsFromCatalogs` directive (`freeze.ts:86`). Under the sync, a drifted build fed the directive the **overlaid** next versions — emission used them too, while the author still read source literals — and, more importantly, two readers of one config inside a single build could disagree, manifesting as peer rules derived from versions the emitted catalogs did not agree with. The split-brain was reachable only for a consumer using **both** the directive and `source:` entries. The final shape is uniformly "as authored": both consumers read the source literals, eliminating the defect class — builds emit exactly what the source says, always. Because this was caught pre-publish, the surface (`syncWorkspaceCatalogs`, `PluginConfig.onCatalogUpdate`, the `CatalogChange`/`CatalogChanges` types, `PluginDeps.cwd`) could be deleted outright rather than deprecated.
...
Workspace version source (complete, on `feat/effected-plugin`): catalog entries can declare `source: "workspace"` to track the local workspace's **next** release versions (current manifest versions overlaid with pending changeset bumps) instead of the npm registry. Two surfaces: the `VersionSource` authoring field on `CatalogPackageSpec`, and — in the `upgrade` CLI, documented in [upgrade-cli.md](upgrade-cli.md) — a second `RegistryResolver` implementation backed by sync `node:fs` reads plus source routing, the release-age exemption and the `--check` drift gate. A third surface, a build-time freeze-path sync with an `onCatalogUpdate` callback, was built and then removed after dogfood adoption by the first consumer — see the builds-never-write rationale under [System architecture](#system-architecture). This implements Section 1 of the cross-repo catalog-automation design (`savvy-web/systems` spec `docs/superpowers/specs/2026-08-21-effected-catalog-automation-design.md`, whose D7 was reversed to match the CLI-as-sole-writer outcome); the local plan was `docs/superpowers/plans/2026-08-21-workspace-version-source.md`. Known limitation: a **prerelease** workspace next version is dropped by `plan`'s stable-only candidate filter (unless the entry's own range is already a prerelease on the same track), so a workspace-sourced entry cannot advance onto a prerelease next release.
...
- `package/src/plugin/allowed-versions.ts` — the `allowedVersionsFromCatalogs` authoring directive, resolved at freeze time into `peerDependencyRules.allowedVersions` and baked into `base`.- `package/src/registry.ts` and `package/src/runtime/strategies/table.ts` — the derived field-to-strategy registry and the strategy table.
