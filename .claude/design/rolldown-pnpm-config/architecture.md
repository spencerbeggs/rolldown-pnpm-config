...
Authoring API (`@public`, build time): authors call `PnpmConfigPlugin({...})` to declare catalogs plus the managed pnpm fields in one canonical, statically analyzable call. Catalogs are inline — `catalogs: { <name>: { packages: { <pkg>: range | { range, peer?, strategy?, source? } } } }`, keyed by catalog name. `source` is `VersionSource` (`"registry" | "workspace"`, absent = registry): where the `upgrade` CLI and the build-time sync resolve the entry's version from. It is deliberately orthogonal to `strategy`, which governs how `peer` is derived from `range` — collapsing the two would drop peer materialization. Like `strategy`, it is CLI/build-side metadata: `normalizeCatalogs` ignores it. `normalizeCatalogs` also skips (never throws on) non-declaration siblings inside the `catalogs` map — an untyped JS consumer or a future reserved key may place a function or `packages`-less object there, and the static discover walk in the CLI skips them identically. The `PluginConfig` shape is a hand-authored interface (one `FieldInput<T>` per field, for rich per-field JSDoc and DX), kept in lockstep with the descriptor table by the compile-time drift guard described below. `defineCatalogs` and `definePlugin` were removed; the catalog types and a pure `normalizeCatalogs` now live in `package/src/catalogs.ts`, and the `PluginConfig`/`FieldInput` types in `package/src/define-plugin.ts`.
...
`PluginConfig.onCatalogUpdate` is an optional top-level callback — `(changes: CatalogChanges) => void | Promise<void>` — fired once per build whose workspace-source sync (below) rewrote at least one catalog entry, after the source write succeeds; never on a no-op. It is deliberately a **sibling** of `catalogs`, not a key inside it: a reserved key inside `Record<string, CatalogDeclaration>` is untypeable and would collide with a real catalog named `"onUpdate"`. The package never interprets the changes — the consuming repo's callback decides (e.g. write a changeset), and its TSDoc requires consumers to gate the callback on their own sync-workflow environment variable, because the callback makes a build mutating and `build:dev`/PR-CI/release-validation builds must all leave the repo untouched.
...
The build-time authoring surface plus the runtime entry are `@public`; everything else — the strategies, the descriptor table, the derived field registry, `freeze`, `ConfigError`, the warning-box formatters and the ctx helpers — is `@internal`. The entry point (`package/src/index.ts`) exports the catalog authoring types including `VersionSource`, `CatalogChange` and `CatalogChanges` — the latter two are the `onCatalogUpdate` payload, which a consumer's typed callback needs by name. The exact `@public` set is the export list of `package/src/index.ts` and the runtime types in `package/src/runtime/types.ts`; treat changes to either as API-surface changes. `createHooks` is `@public` and now has a required third `name: string` parameter — this is a breaking change for any direct consumer of the runtime entry.
...
`PnpmConfigPlugin(config)` is constructed with the inline `PluginConfig` object and serves two virtual modules. The Effect `freeze` runs once and is memoized on the plugin closure, because the bundler invokes the plugin across several passes and the validate-freeze-manifest work must not repeat. See `package/src/plugin/index.ts`. `PluginDeps` (the internal test seam) carries an optional `cwd` alongside the injectable `freeze`, rooting the workspace-source sync and patch discovery (defaults to `process.cwd()`).

Inside that same memoized path, **before** `freeze`, the plugin runs `syncWorkspaceCatalogs(config, cwd)` (`package/src/plugin/workspace-sync.ts`): when any catalog entry declares `source: "workspace"`, it locates the config source file (reusing the CLI's `select-file` autodetect), statically discovers the workspace-sourced entries (the CLI's `discover`), reads the workspace's next release versions (`readWorkspaceVersions`, see [upgrade-cli.md](upgrade-cli.md)) and rewrites any drifted range literal in the config **source** (oxc byte spans via `applyEdits`, formatting-preserving), recomputing `lock`/`lock-minor` peers offline — an `interop` entry's range is rewritten but its group-derived peer is left to the `upgrade` CLI. It then overlays the same values into the in-memory config so **this** build's emitted catalogs already carry them — one-build convergence (spec D7): resolve-at-emit-only never converges because the source literal must move too, or drift is re-detected on every build. The sync degrades to a no-op when the config file cannot be located or a package is not publishable, but a failed write of the config source fails the build loudly. After a successful rewrite (and only then), `onCatalogUpdate` fires with the `CatalogChanges`.
...
Workspace version source (complete, on `feat/effected-plugin`): catalog entries can declare `source: "workspace"` to track the local workspace's **next** release versions (current manifest versions overlaid with pending changeset bumps) instead of the npm registry. Three surfaces: the `VersionSource` authoring field on `CatalogPackageSpec`; a second `RegistryResolver` implementation backed by sync `node:fs` reads plus source routing, the release-age exemption and the `--check` drift gate in the `upgrade` CLI (documented in [upgrade-cli.md](upgrade-cli.md)); and the build-time `syncWorkspaceCatalogs` + `onCatalogUpdate` pipeline described under [System architecture](#system-architecture). This implements Section 1 of the cross-repo catalog-automation design (`savvy-web/systems` spec `docs/superpowers/specs/2026-08-21-effected-catalog-automation-design.md`); the local plan was `docs/superpowers/plans/2026-08-21-workspace-version-source.md`. Known limitation: a **prerelease** workspace next version is dropped by `plan`'s stable-only candidate filter (unless the entry's own range is already a prerelease on the same track), so a workspace-sourced entry cannot advance onto a prerelease next release.
...
- `package/src/plugin/workspace-sync.ts` — the build-time workspace-source sync (`syncWorkspaceCatalogs`), run in the memoized plugin path before `freeze`; fires `onCatalogUpdate`.
