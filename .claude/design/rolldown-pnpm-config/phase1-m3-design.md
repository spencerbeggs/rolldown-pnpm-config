...
> **Known local-only gate (skips cleanly in CI):** the parity `*.int.test.ts` suite is a *differential* test that loads Silk's built artifacts (`pnpmfile.cjs` + `catalogs/generated.js`) from the sibling repo. The path defaults to the canonical local checkout (`/Users/spencer/workspaces/savvy-web/pnpm-plugin-silk/dist/dev/pkg`) and is overridable via the `SILK_DIST` env var. Both oracle loaders (`loadSilkOracle`, `loadSilkCatalogs`) guard with `existsSync` and return `null` when the artifacts are absent, so the oracle-dependent tests skip cleanly rather than crash: the differential cases are `it.runIf(silk !== null)`, `base-parity` is `describe.skipIf(oracle === null)`, and the `oracle is present` guard is `it.skipIf(process.env.CI)` — it still fails a *local* run that forgot to build Silk, but is skipped in CI where the sibling repo is intentionally absent. The warning-presence tests need no oracle and run everywhere. Net: full battery runs locally (build Silk first: `pnpm -C <silk> run build`, or point `SILK_DIST` at a checkout); the suite skips — never fails — in a bare CI checkout. This matches the self-contained-in-rolldown design choice (§6).
